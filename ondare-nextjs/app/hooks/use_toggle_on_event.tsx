...
export function useToggleOnEvent<T extends HTMLElement = HTMLElement>({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      switch (trigger) {
        case "scroll-down":
          if (y > lastY.current) target.classList.add(safeClassName);
          else target.classList.remove(safeClassName);
          break;
        ...
        case "scroll-past":
          if (y > amount) target.classList.add(safeClassName);
          else target.classList.remove(safeClassName);
          break;
      }
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (trigger === "enter-viewport" && entry.isIntersecting) {
              entry.target.classList.add(safeClassName);
            }
    ...
    const handleClick = () => {
      if (trigger === "click") {
        target.classList.toggle(safeClassName);
      }
    };
  ...
  }, [trigger, safeClassName, amount, targetRef, triggerRef]);
